...
public class DoctorLoginScreen
{
    ...
    private Label LBL1 = new Label("Doctor Login Page.");
    ...
    {
        DoctorLoginScreen DLS = new DoctorLoginScreen();
        return DLS.CreateNewScene(stage);
    }
    ...
    private void HandleButtonsAction(Stage stage) {
        ...
        BtnToGoNextScreen.setOnAction(e -> {
            ...
            else if(!Doctor.IsDoctorUserAlreadyExists(Txt1.getText(),Txt2.getText()))
            {
                Message.setTextFill(Color.RED);
                Message.setText("Invalid email or password");
            }
        ...
        {
            Scene scene = DoctorSignUpScreen.DoctorSignUpAccount(stage);
            stage.setScene(scene);
        });
    }
}
